...
const char *paramValues_ins[2],*paramValues_upd[2], *paramValue_del[1], *paramValue_sel[1];
int paramLengths_ins[2], paramLengths_upd[2], paramLength_del[1], paramLength_sel[1];
int paramFormats_ins[2], paramFormats_upd[2], paramFormat_del[1], paramFormat_sel[1];
...
uint32_t binaryIntVal;
    ...
    if (argc > 1)
        ...
        conninfo = "host=10.136.2.133 dbname=lwwtest user=lww password=lww port=5432";
   /* 连接数据库服务器*/
...
*/
    /*insert */
 binaryIntVal = htonl((uint32_t) 77);
 paramValues_ins[0] = (char*) &binaryIntVal;
 paramLengths_ins[0] = sizeof(binaryIntVal);
 paramFormats_ins[0] = 1; 

paramValues_ins[1]="testCodeInsert";
paramLengths_ins[1]=sizeof(paramValues_ins[1]);
paramFormats_ins[1]= 1;
