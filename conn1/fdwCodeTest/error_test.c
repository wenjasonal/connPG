...
//uint32_t binaryIntVal;
    ...
    if (argc > 1)
        ...
        conninfo = "host=10.136.2.85 dbname=lwwtest user=lww password=lww port=5432";
   /* 连接数据库服务器*/
...
// res = PQexec(conn,"insert into organ values ('80','testCode')");
 res = PQexec(conn,"insert into organ  select generate_series(1, 1000000)");
...
/*
paramValues_ins[0]="1";
paramValues_ins[1]="10000";
res = PQexecParams(conn,
                  "insert into organ select generate_series($1, $2)",
                  2,
                  NULL,
                  paramValues_ins,
                  NULL,
...
/*select*/
/*
paramValues_sel[0] = "77";
res = PQexecParams(conn,
                   "select * from organ where organcode=$1",
                   1,
                   NULL,
                   paramValues_sel,
                   NULL,
